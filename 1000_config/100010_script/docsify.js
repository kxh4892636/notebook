(function () {
  ...
  var Tokenizer = /*@__PURE__*/ (function () {
    ...
    Tokenizer.prototype.strong = function strong(src, maskedSrc, prevChar) {
      ...
      if (
        match &&
        (!match[1] ||
          (match[1] && (prevChar === "" || this.rules.inline.punctuation.exec(prevChar))))
      ) {
        maskedSrc = maskedSrc.slice(-1 * src.length);
        var endReg =
          match[0] === "**" ? this.rules.inline.strong.endAst : this.rules.inline.strong.endUnd;
    ...
    Parser.prototype.parse = function parse(tokens, top) {
      ...
      for (i = 0; i < l; i++) {
        ...
        switch (token.type) {
          ...
          case "html": {
            // parse inline content if parameter markdown=1
  ...
  ********************************************** */
  ...
  var linkCompiler = function (ref) {
    ...
    return (renderer.link = function (href, title, text) {
      ...
      if (!isAbsolutePath(href) && !compilerClass._matchNotCompileLink(href) && !config.ignore) {
        ...
        attrs.push(
          href.indexOf("mailto:") === 0 ? "" : linkRel !== "" ? ' rel="' + linkRel + '"' : ""
        );
      }
          ...
          if (this$1.isHTML) {
            ...
            prerenderEmbed(
              ...
              function (tokens) {
                ...
                html = this$1.isRemoteUrl ? purify.sanitize(html, { ADD_TAGS: ["script"] }) : html;
                callback();
                next();
              }
            );
  ...
  function loadNested(path, qs, file, next, vm, first) {
    ...
    get(vm.router.getFile(path + file) + qs, false, vm.config.requestHeaders).then(
      next,
      function (_) {
  ...
  // This is deprecated, kept for backwards compatibility. Remove in next
