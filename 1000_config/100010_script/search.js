(function () {
  ...
  function saveData(maxAge, expireKey, indexKey) {
    // 3600000 'docsify.search.expires' 'docsify.search.index'
    const server = indexedDB.open("docsify");
    server.onsuccess = () => {
      const transaction = server.result.transaction("docsify", "readwrite");
      const docsifyObjcet = transaction.objectStore("docsify");
      docsifyObjcet.clear();
      docsifyObjcet.add({ id: indexKey, value: JSON.stringify(INDEXS) });
      docsifyObjcet.add({ id: expireKey, value: Date.now() + maxAge });
    };
  }
  ...
  function init(config, vm) {
    ...
    INDEXS;
    const server = indexedDB.open("docsify");
    server.onupgradeneeded = () => {
      const db = server.result;
      db.createObjectStore("docsify", { keyPath: "id" });
    };
    server.onsuccess = async () => {
      const db = server.result;
      const transaction = server.result.transaction("docsify", "readwrite");
      const docsifyObjcet = transaction.objectStore("docsify");
      const requestExpire = docsifyObjcet.get(expireKey);
      const requestIndex = docsifyObjcet.get(indexKey);
      requestExpire.onsuccess = () => {
        if (!requestExpire.result) isExpired = true;
        else isExpired = requestExpire.result.value < Date.now();
      };
      requestIndex.onsuccess = () => {
        INDEXS = JSON.parse(requestIndex.result ? requestIndex.result.value : "{}");
        if (!requestIndex.result || isExpired) {
          INDEXS = {};
        } else if (!isAuto) {
          return;
        }
        ...
        paths.forEach(function (path) {
          ...
          Docsify.get(vm.router.getFile(path), false, vm.config.requestHeaders).then(function (
            result
          ) {
