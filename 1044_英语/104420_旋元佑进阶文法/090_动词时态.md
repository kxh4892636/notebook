...
```json
// now 暗示为现在时间
// has been 使用现在式, done 形容词补语, 使用过去分词表示被动
...
```json
// now 暗示为现在时间, for 使用完成式
// has been 使用现在式, running 形容词补语, 使用现在分词表示持续进行
...
```json
// by the middle of this century 表示未来时间, 使用完成式
...
```json
// In two more minutes 表示未来时间, for three hours 表示使用完成式
...
```json
// 不佳
// that 子句表明过去时间, 但建筑物建造时间应在地震发生之前, 不应该使用简单式
